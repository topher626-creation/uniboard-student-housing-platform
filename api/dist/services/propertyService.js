...
export class PropertyService {
    async createProperty(providerId, data) {
    ...
    async getProperties(filters = {}) {
        ...
        if (filters.nearestUniversity)
            where.nearestUniversity = filters.nearestUniversity;
        if (filters.location)
    ...
    async updateProperty(id, providerId, data) {
        const property = await Property.findByPk(id);
        if (!property) {
            throw new AppError(404, 'Property not found');
        }
    ...
    async deleteProperty(id, providerId) {
        const property = await Property.findByPk(id);
        if (!property) {
            throw new AppError(404, 'Property not found');
        }
